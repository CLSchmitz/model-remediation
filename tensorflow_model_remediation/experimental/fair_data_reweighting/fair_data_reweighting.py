...
def reweight_data(data: tf.data.Dataset,
  ...
  Takes in a dataset to be reweighted and a MetricByFeatureSlice object,
  such as those generated by //TODO TF Fairness Indicators, representing
  performance of a model against a given metric across protected feature slices.
  Resamples the dataset to better represent underrepresented slices, parametrized
  by gamma (proportion of samples to take from fair distribution vs original
  distribution) and beta (how strongly poorer-performing slices are upsampled
  within the fair distribution).

  Arguments:
    data: A tf.data.Dataset to be reweighed.
    eval_metrics: The MetricByFeatureSlice object.
    gamma: The quantity governing the sampling factor, between 0 (sample only 
      from original dataset) and 1 (sample only from fair data distribution).
    beta: The temperature parameter for expoential weighting, affecting how 
      strongly poorer-performing slices are upsampled within the fair data 
      distribution, between 0 (no upsampling) and infinity.
